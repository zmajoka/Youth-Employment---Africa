...
preserve
    ...
    * (add any other enterprise-level vars you need - sector, formality, etc.)
    
    * Rename with _2018 suffix so they don't clash on merge later
    rename revenue         revenue_2018
    rename expenses        expenses_2018
    rename profit          profit_2018
    rename value_total     value_total_2018
    rename num_emp         num_emp_2018
    rename num_emp_tot     num_emp_tot_2018
    rename num_emp_child   num_emp_child_2018
    rename hires_nonhh_workers hires_nonhh_workers_2018
    rename n_enterprises_prop  n_enterprises_prop_2018
    
    * Save
	count
    save "${intermediate}/SEN_enterprises_2018.dta", replace
    di as text _n "Saved enterprise-level 2018 file: `r(N)' rows"
restore
...
gen byte is_dependent = (age < 15 | age > 64) if !missing(age)
